...
%% File    : erlog_db_rkv.erl
%% Purpose : Interface to an erlog database built with a raptor_kv

%% The database is a raptor_kv table where the key is the functor pair {Name,Arity}.
...
add_built_in(Db, Functor) ->
    raptor_kv:insert(Db, {Functor,built_in}),
    Db.
...
add_compiled_proc(Db, Functor, M, F) ->
    case raptor_kv:lookup(Db, Functor) of
	[{_,built_in}] -> error;
	_ ->
	    raptor_kv:insert(Db, {Functor,code,{M,F}}),
	    {ok,Db}
    end.
...
asserta_clause(Db, Functor, Head, Body) ->
	...
	[{_,code,_}] -> error;
	[{_,clauses,Tag,Cs}] ->
	    raptor_kv:insert(Db, {Functor,clauses,Tag+1,[{Tag,Head,Body}|Cs]}),
	    {ok,Db};
	[] ->
	    raptor_kv:insert(Db, {Functor,clauses,1,[{0,Head,Body}]}),
	    {ok,Db}
    end.
...
assertz_clause(Db, Functor, Head, Body) ->
	...
	[{_,code,_}] -> error;
	[{_,clauses,Tag,Cs}] ->
	    raptor_kv:insert(Db, {Functor,clauses,Tag+1,Cs ++ [{Tag,Head,Body}]}),
	    {ok,Db};
	[] ->
	    raptor_kv:insert(Db, {Functor,clauses,1,[{0,Head,Body}]}),
	    {ok,Db}
...
retract_clause(Db, Functor, Tag) ->
	...
	[{_,clauses,Nt,Cs}] ->
	    raptor_kv:insert(Db, {Functor,clauses,Nt,lists:keydelete(Tag, 1, Cs)}),
	    {ok,Db};
	[] -> {ok,Db}				%Do nothing
    end.
...
get_interpreted_functors(Db) ->
    raptor_kv:foldl(fun ({Func,clauses,_,_}, Fs) -> [Func|Fs];
		  (_, Fs) -> Fs
	      end, [], Db).
