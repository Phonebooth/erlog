...
%% File    : erlog_db_map.erl
%% Purpose : Interface to an erlog database built with map.

%% The database is a map where the key is the functor pair {Name,Arity}.
...
add_compiled_proc(Db, Functor, M, F) ->
    ...
    ?RET_CATCH(maps:update_with(Functor, Fun, Code, Db)).
...
asserta_clause(Db, Functor, Head, Body) ->
    ...
    ?RET_CATCH(maps:update_with(Functor, Fun, {clauses,1,[{0,Head,Body}]}, Db)).
...
abolish_clauses(Db, Functor) ->
	...
	{ok,{code,_}} -> {ok,maps:remove(Functor, Db)};
	{ok,{clauses,_,_}} -> {ok,maps:remove(Functor, Db)};
	error -> {ok,Db}			%Do nothing
    end.
...
get_interpreted_functors(Db) ->
    maps:fold(fun (Func, {clauses,_,_}, Fs) -> [Func|Fs];
		  (_, _, Fs) -> Fs
	      end, [], Db).
